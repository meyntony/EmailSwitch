...
namespace EmailSwitch.Tests
{
	...
	public sealed class EmailProviderTests
	{
		...
		[Fact]
		public void Brevo_is_appended_at_the_next_free_value()
		{
			Assert.Equal(3, (int)EmailProvider.Brevo);

			// Catches a member added anywhere but the end: a new one inserted above Brevo would shift
			// it without changing any assertion above. Update this count deliberately when appending a
			// provider - it is meant to fail first and make the renumbering question explicit.
			Assert.Equal(4, Enum.GetValues<EmailProvider>().Length);
		}
	}
